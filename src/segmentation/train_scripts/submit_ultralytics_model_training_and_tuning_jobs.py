...
class UltraalyticsJobSubmitter:
    ...
    def create_and_submit_slurm_job(model_config: str, train_model: bool = True) -> None:
        os.environ["OUTPUT_DIR"] = str(PROJECT_ROOT / "src/segmentation/notebooks/ultralytics/train_logs")
        os.environ["MODEL_CONFIG"] = model_config

        if train_model:
            template_path = str(PROJECT_ROOT / "cluster_instructions/launch_scripts/sbatch_gpu_train_ultralytics.sh")
            os.environ["JOB_NAME"] = f"train_{model_config.replace('.', '_')}"
        else:
            template_path = str(PROJECT_ROOT / "cluster_instructions/launch_scripts/sbatch_gpu_tune_ultralytics.sh")

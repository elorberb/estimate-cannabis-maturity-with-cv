...
def get_color_ratios_from_dict(json_data):
    return {
...
def get_color_ratios_from_list(json_data):
    num = len(json_data)
...
def get_color_ratios(json_data):
    if isinstance(json_data, dict) and "num_pistils" in json_data:
        return get_color_ratios_from_dict(json_data)
    if isinstance(json_data, list) and json_data:
        return get_color_ratios_from_list(json_data)
...
def collect_image_entries(numbered_folder_path):
    entries = []
    for item in os.listdir(numbered_folder_path):
        ...
        if os.path.exists(img_json_path):
            entries.append(get_color_ratios(load_json(img_json_path)))

    return entries


def aggregate_folder_json(numbered_folder_path):
    image_entries = collect_image_entries(numbered_folder_path)
    ...
    save_json(aggregated, os.path.join(folder_analysis_dir, "aggregated_pistils_color_ratios.json"))

    return aggregated


def process_image_entries(numbered_folder_path, day_folder, subfolder, numbered_folder):
    entries = []
    ...
    for day_folder in day_folders:
        ...
        for subfolder in ["greenhouse"]:
            ...
            for numbered_folder in os.listdir(subfolder_path):
                ...
                data_per_image.extend(process_image_entries(
...
if __name__ == "__main__":
    ...
    data_per_image, data_per_folder = collect_pistils_data(base_path, EXP_DAY_FOLDERS)
    save_data_to_csv(data_per_image, os.path.join(output_folder, "collected_pistils_results_per_image.csv"))
    save_data_to_csv(data_per_folder, os.path.join(output_folder, "collected_pistils_results_per_folder.csv"))
